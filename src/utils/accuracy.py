...
class AccuracyTracker:
    ...
    def __init__(self):
        ...
        self.current_shape_name = "Shape"  # Default shape name
    ...
    def set_current_shape(self, shape_name: str):
        """Set the current shape name for display in the accuracy panel."""
    ...
    def draw_accuracy_panel(self, surface: pygame.Surface, position: Tuple[int, int], 
        ...
        shape_font = pygame.font.SysFont(None, scaled_font_sizes['medium'])
        ...
        # Draw current shape name
        shape_surf = shape_font.render(f"Current: {self.current_shape_name}", True, Config.BLUE)
        shape_rect = shape_surf.get_rect(midtop=(x + width//2, y + 35))
        surface.blit(shape_surf, shape_rect)
        ...
        grade_rect = grade_surf.get_rect(center=(x + width//2, y + 70))  # Move down slightly
        ...
        feedback_rect = feedback_surf.get_rect(midtop=(x + width//2, y + 150))  # Move down slightly
        surface.blit(feedback_surf, feedback_rect)
        
        # Draw detailed metrics if requested
        if show_details and height > 200:
            detail_y = y + 180  # Move down slightly
            line_height = 25
